...
class NeuralRayBaseRenderer(nn.Module):
    ...
    def network_rendering(self, prj_dict, que_pts, que_dir, is_fine):
        if is_fine:
            pixel_colors, weight = self.fine_agg_net(prj_dict, que_pts, que_dir)
        else:
            pixel_colors, weight = self.agg_net(prj_dict, que_pts, que_dir)

        return pixel_colors, weight
    ...
    def render_by_depth(self, que_depth, que_imgs_info, ref_imgs_info, is_train, is_fine):
        ...
        pixel_colors_nr, hit_prob_nr = self.network_rendering(prj_dict,que_pts, que_dir, is_fine)
        hit_prob_nr = hit_prob_nr.permute(2,0,1)
...
class NeuralRayFtRenderer(NeuralRayBaseRenderer):
    default_cfg={
        ...
        # training related
        'gen_cfg': None, # 'configs/train/gen/ft_lr_insert_lego.yaml'
        "use_validation": True,
        "validate_initialization": True, # visualize rendered images of inited insert on the val set
...
name2network={
    'insert_gen': NeuralRayGenRenderer,
    'insert_ft': NeuralRayFtRenderer,
}
