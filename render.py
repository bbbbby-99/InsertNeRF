...
def render_video_gen(database_name: str,
                     cfg_fn='configs/gen_lr_insert.yaml',
...
if __name__=="__main__":
    ...
    parser.add_argument('--database_name', type=str, default='', help='<dataset_name>/<scene_name>/<scene_setting>')
    parser.add_argument('--dataset', type=str, default='llff')
    parser.add_argument('--cfg', type=str, default='configs/train/gen/insert_gen_depth_train.yaml', help='config path of the renderer')
    ...
    parser.add_argument('--depth', action='store_true', dest='depth', default=True)
    # parser.add_argument('--overlap', action='store_true', dest='overlap', default=False)
    flags = parser.parse_args()
    if flags.dataset == 'llff':
        for i in ['llff_colmap/trex/high', 'llff_colmap/fern/high', 'llff_colmap/flower/high','llff_colmap/leaves/high','llff_colmap/room/high','llff_colmap/fortress/high', 'llff_colmap/horns/high','llff_colmap/orchids/high']:
            flags.database_name = i
        ...
        for i in ['nerf_synthetic/chair/white_400', 'nerf_synthetic/drums/white_400', 'nerf_synthetic/ficus/white_400','nerf_synthetic/hotdog/white_400','nerf_synthetic/materials/white_400','nerf_synthetic/mic/white_400', 'nerf_synthetic/ship/white_400','nerf_synthetic/lego/white_400']:
            flags.database_name = i
            if flags.render_type=='gen':
                render_video_gen(flags.database_name, cfg_fn=flags.cfg, pose_type=flags.pose_type, pose_fn=flags.pose_fn,
                                 render_depth=flags.depth, ray_num=flags.ray_num, rb=flags.rb,re=flags.re)
        ...
        for i in ['dtu_test/birds/black_400', 'dtu_test/bricks/black_400', 'dtu_test/snowman/black_400','dtu_test/tools/black_400']:
            flags.database_name = i
            if flags.render_type=='gen':
                render_video_gen(flags.database_name, cfg_fn=flags.cfg, pose_type=flags.pose_type, pose_fn=flags.pose_fn,
                                 render_depth=flags.depth, ray_num=flags.ray_num, rb=flags.rb,re=flags.re)
